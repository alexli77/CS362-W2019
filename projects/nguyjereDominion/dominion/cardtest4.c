...
int main (){
    ...
    //check if the card effects are correct
    printf("Check Player 0 Gains 1 Card: ");
    compareStates(state.handCount[p0],stateOriginal.handCount[p0]+1);
    printf("Check If Valid Card Gained: ");
    compareStates(1,(handCard(handpos,&state) != adventurer));
    //printf("%i\n", handCard(handpos,&state) != adventurer);
    printf("Check Player 0 Pile if 1 Card Taken: ");
    compareStates(state.deckCount[p0],stateOriginal.deckCount[p0]-1);
    ...
    //check if the 2nd player is unaffected
